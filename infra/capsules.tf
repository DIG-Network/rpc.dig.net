...
#   2. Objects here are keyed `{store_hex}/{root_hex}.dig` to match dig-node's `CapsuleKey::
#      module_path` (dig-node-core `capsule_key.rs`, `CACHED_MODULE_EXT`). The hub's bucket uses
#      the SAME `.dig` suffix — one canonical extension ecosystem-wide — so these are two buckets
#      with two lifecycles, not two key shapes. The directory is still named `modules/`; only the
#      file extension is `.dig`, and that split is what keeps luring the key back to `.module`.
